...
export const cartSlice = createSlice({
  ...
  reducers: {
    add_cart_product: (state, { payload }) => {
      ...
      const availableQuantity = Number(payload.quantity || 0);
      if (payload.isVariable && !payload.variation_id) {
        notifyError("Please select product options first.");
        state.orderQuantity = 1;
        return;
      }
      if (payload.status === "out-of-stock" || availableQuantity <= 0) {
      ...
      if (!isExist) {
        ...
        state.cart_products.map((item) => {
          if (item._id === payload._id) {
            if (availableQuantity >= item.orderQuantity + state.orderQuantity) {
              item.orderQuantity =
                state.orderQuantity !== 1
    ...
    increment: (state, { payload }) => {
      const availableQuantity = Number(payload?.quantity || 0);
      if (availableQuantity && state.orderQuantity >= availableQuantity) {
        notifyError("No more quantity available for this product!");
        return;
      }
    ...
    remove_product: (state, { payload }) => {
      state.cart_products = state.cart_products.filter(
        (item) => item._id !== payload.id && item._id !== payload._id
      );
      setLocalStorage("cart_products", state.cart_products);
